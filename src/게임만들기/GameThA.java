    ...
    public void run() {
        ...
        while(true) {
            try {
                ...
                special = (int) (Math.random() * 20); // 0 ~ 19의 값이 만들어진다 (5% 확률로 궁극기 공격)
                if(normal == 1) {
                    System.out.println("물리공격 : " + wizard.name + "에게 " +
                            warrior.pAttack() + "의 데미지를 입혔습니다.");
                    endGame = wizard.setDamage(warrior.pAttack());
                } else {
                    System.out.println("마법공격 : " + wizard.name + "에게 " +
                            warrior.mAttack() + "의 데미지를 입혔습니다.");
                    endGame = wizard.setDamage(warrior.mAttack());
                }
                if(special == 10) { // 5% 확률로 발동
                    System.out.println(warrior.name + "의 궁극기 발동 !!!  " + wizard.name + "에게 "
                    + warrior.ultimate() + "의 데미지를 입혔습니다.");
