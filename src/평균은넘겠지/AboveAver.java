...
public class AboveAver {
    ...
    // 각 케이스에 대한 학생 수와 그리고 각각의 성적을 입력 받음
    // 입력 받은 성적에 대한 평균 구하기 (먼저 총점을 구한다)
    // 평균을 넘는 학생 수 구하기 (소수점 이하 3자리까지로 변환해서 반환)
    static double overRate() {
        ...
        for(int i = 0; i < score.length; i++) {
            score[i] = sc.nextInt();
            total += score[i]; // 입력 받은 학생 수만큼 성적을 누적해 총점을 구함
        }
        double aver = (double)(total / cnt);
        for(int e : score) {    // e값은 개별 학생의 성적
            if(e > aver) overCnt++; // 평균을 넘는 학생 수 구하기
        }
        return (double) overCnt / cnt * 100; // 백분율로 변경하기 위해 100 곱해준다.
    }
}
