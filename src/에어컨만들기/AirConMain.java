...
public class AirConMain {
    public static void main(String[] args) throws InterruptedException {
        ...
        if(isOn.equalsIgnoreCase("yes")) {
            airCon.setPower(true); // 에어컨 전원 켜기
            airCon.setAirConState(); // 에어컨의 온도 및 바람 세기 설정
            while(true) {
                ...
                if (isSetTemp) {
                    if(airCon.isHeater()) airCon.setSetCurrTemp(1);
                    if(airCon.isCooler()) airCon.setSetCurrTemp(-1);
                    airCon.airConInfo();
                    isSetTemp = false;
                    elapsedTime = 0;
                }
